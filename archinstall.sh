...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. Script can install for a user already existing, but it will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\n Script will NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that script will change $name's password to the one you just gave." 14 70
...
maininstall() {
	...
	whiptail --title "Apps Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 9 70
	installpkg "$1"
}
...
gitmakeinstall() {
	...
	whiptail --title "Apps Installation" \
...
rm -rf "/home/$name/.git/" "/home/$name/README.md" "/home/$name/LICENSE" "/home/$name/.bash*"
...
echo "%wheel ALL=(ALL:ALL) ALL" >/etc/sudoers.d/00-koloika55-wheel-can-sudo
echo "%wheel ALL=(ALL:ALL) NOPASSWD: /usr/bin/shutdown,/usr/bin/reboot,/usr/bin/systemctl suspend,/usr/bin/mount,/usr/bin/umount,/usr/bin/pacman -Syu,/usr/bin/pacman -Syyu,/usr/bin/pacman -Syyu --noconfirm,/usr/bin/pacman -Syyuw --noconfirm,/usr/bin/pacman -S -u -y --config /etc/pacman.conf --,/usr/bin/pacman -S -y -u --config /etc/pacman.conf --" >/etc/sudoers.d/01-koloika55-cmds-without-password
echo "Defaults editor=/usr/bin/nvim" >/etc/sudoers.d/02-koloika55-visudo-editor
